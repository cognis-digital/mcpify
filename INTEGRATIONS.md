...
**mcpify** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `mcpify` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
mcpify ... --format json | cognis-connect emit --to sigma   # Sigma rules
mcpify ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
mcpify ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `mcpify` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="mcpify") for rec in records]   # records = your JSON output
